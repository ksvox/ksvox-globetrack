...
function getFallbackTracks(countryCode, limit = 10) {
    ...
    for (let i = 0; i < limit; i++) {
        if (i < base.length) {
            ...
            results.push({
                rank: num,
                title: `Global Chart Track #${num}`,
                artist: `Popular Global Artist ${num}`,
                album: `Hit Album Vol.${Math.ceil(num / 5)}`,
                img: `https://placehold.co/300x300/181818/1db954?text=%23${num}`,
                previewUrl: null,
                url: 'https://open.spotify.com'
            });
        }
 ...
 * 想定外のエラーでも必ず200 + フォールバックデータを返す安全設計。
 */
export default async function handler(req, res) {
    ...
    try {
        ...
        // トークンが無い場合やプレイリストID未指定の場合は即座にデモデータを返す
        if (!token || !playlistId) {
        ...
        // Spotify APIでエラー（403/404等）が発生した場合はデモデータに安全に切り替え
        if (!spotifyRes.ok) {
            console.warn(`Spotify API responded with status ${spotifyRes.status}. Switching to fallback data.`);
            const demoData = getFallbackTracks(country, limitNum);
        ...
        // ここに来るのは本当に想定外のエラーのみ。それでも画面を落とさずデモデータで応答する。
        console.error('Unexpected error in /api/chart handler:', error);
        try {
            ...
            return res.status(500).json({ error: 'Internal server error', tracks: [] });
        }
    }
              }
