...
export default async function handler(req, res) {
  ...
  if (chartType === 'billboard') {
    try {
      const itunesRes = await fetch(`https://itunes.apple.com/${countryCode.toLowerCase()}/rss/topsongs/limit=${maxLimit}/json`);
      if (itunesRes.ok) {
        ...
        const tracks = entries.map((item, index) => ({
          ...
          url: item.link?.[0]?.attributes?.href || `https://open.spotify.com/search/${encodeURIComponent((item['im:name']?.label || '') + ' ' + (item['im:artist']?.label || ''))}`
        }));
